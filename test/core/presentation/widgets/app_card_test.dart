...
void main() {
  group('AppCard', () {
    testWidgets('Material spans the whole card, not the padded content box '
        ...
        'home macro totals rendered "89/79 g" as "39/79 g")', (tester) async {
      const radius = 20.0;
      const padding = EdgeInsets.all(16);

      await tester.pumpWidget(
        _wrap(const AppCard(borderRadius: radius, padding: padding, child: SizedBox(width: 100, height: 40))),
      );

      final cardSize = tester.getSize(find.byType(AppCard));
      final materialWidget = find.descendant(
        of: find.byType(AppCard),
        matching: find.byWidgetPredicate((w) => w is Material && w.clipBehavior == Clip.none),
      );
      ...
      expect(
        ...
        Size(cardSize.width - 2 * Dimens.hairline, cardSize.height - 2 * Dimens.hairline),
      );
    });

    testWidgets('padding still spaces the child away from the card edge', (tester) async {
      const padding = EdgeInsets.all(16);
