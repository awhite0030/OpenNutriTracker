...
WeightLogEntity _entry(DateTime date, double weightKg) => WeightLogEntity(date: date, weightKg: weightKg);
...
void main() {
  testWidgets('WeightTrendChart draws all points across a DST transition without gaps', (tester) async {
    // The chart computes `windowStart` relative to `DateTime.now()`.
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);

    // Let's just generate entries for the last 200 days to ensure any
    // DST transition inside the window is caught.
