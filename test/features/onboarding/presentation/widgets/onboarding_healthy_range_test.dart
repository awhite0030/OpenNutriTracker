...
void main() {
  Future<void> pumpPage(WidgetTester tester, {void Function(double? target)? onTarget}) async {
      ...
      MaterialApp(
        ...
        home: Scaffold(
          body: OnboardingBodyMeasurementsPageBody(
            setButtonContent: (_, _, _, target, _, _, _) => onTarget?.call(target),
          ),
        ),
      ),
    );
  ...
  Future<void> enterHeightAndWeight(WidgetTester tester, {required String height, required String weight}) async {
  ...
  testWidgets('no card until both height and weight are entered', (tester) async {
    ...
    expect(find.byType(ActionChip), findsNothing, reason: 'the suggestion depends on the current weight too');
  });

  testWidgets('shows the range and its source once both are entered', (tester) async {
    await pumpPage(tester);
  ...
  testWidgets('chip offers the midpoint when the weight is above the band', (tester) async {
    await pumpPage(tester);
    await enterHeightAndWeight(tester, height: '180', weight: '95');

    // Midpoint for 180 cm is BMI 21.7 -> 70.3 kg.
    expect(find.text(l10nEn.onboardingHealthyRangeApply('70.3 kg')), findsOneWidget);
  });

  testWidgets('chip offers the current weight when already healthy', (tester) async {
    await pumpPage(tester);
  ...
  testWidgets('tapping the chip fills the target field', (tester) async {
    ...
    expect(tester.widget<TextField>(find.byType(TextField).at(2)).controller?.text, '70.3');
  });

  testWidgets('the card links to the sources behind the calculation', (tester) async {
    await pumpPage(tester);
    await enterHeightAndWeight(tester, height: '180', weight: '95');
  ...
  testWidgets('focusing the target field scrolls the suggestion into view', (tester) async {
    await pumpPage(tester);
    await enterHeightAndWeight(tester, height: '180', weight: '95');
  ...
  testWidgets('an implausible weight asks for confirmation on blur', (tester) async {
    await pumpPage(tester);
    await enterHeightAndWeight(tester, height: '180', weight: '5');

    expect(find.text(l10nEn.onboardingImplausibleTitle), findsNothing, reason: 'not while the field still has focus');
  ...
  testWidgets('keeping an implausible value accepts it and does not re-ask', (tester) async {
    bool? lastActive;
    await tester.pumpWidget(
      MaterialApp(
        localizationsDelegates: const [S.delegate],
        supportedLocales: S.supportedLocales,
        home: Scaffold(
          body: OnboardingBodyMeasurementsPageBody(setButtonContent: (active, _, _, _, _, _, _) => lastActive = active),
        ),
      ),
    );
    await enterHeightAndWeight(tester, height: '180', weight: '5');
    ...
    expect(lastActive, isTrue, reason: '5 kg clears the hard bounds, so the page can still proceed');
  ...
  group('imperial units get the same checks as metric', () {
    testWidgets('an incomplete ft/in height reports its error on blur', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          localizationsDelegates: const [S.delegate],
          supportedLocales: S.supportedLocales,
      ...
      await tester.pump();
      expect(find.text(l10nEn.onboardingWrongHeightLabel), findsNothing, reason: 'still typing');
    ...
    testWidgets('an incomplete stones weight reports its error on blur', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          localizationsDelegates: const [S.delegate],
          supportedLocales: S.supportedLocales,
    ...
    testWidgets('an incomplete stones target blocks progress and reports its error', (tester) async {
      bool? lastActive;
    ...
    testWidgets('an implausible ft/in height asks for confirmation', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          localizationsDelegates: const [S.delegate],
          supportedLocales: S.supportedLocales,
    ...
    testWidgets('an implausible stones weight asks for confirmation', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          localizationsDelegates: const [S.delegate],
          supportedLocales: S.supportedLocales,
